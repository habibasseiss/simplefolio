...
import { createTransactionAction, deleteTransactionAction } from '../transaction.actions';
...
describe('Transaction Actions', () => {
  beforeEach(() => {
    ...
    vi.mocked(accountRepo.findAccountById).mockResolvedValue({ id: 'acc-1', name: 'TFSA', currency: 'CAD', userId: 'user-1', createdAt: new Date(), updatedAt: new Date() });
    vi.mocked(finance.getFinanceProvider).mockReturnValue({
      getSymbolInfo: vi.fn().mockResolvedValue({ name: 'Apple Inc.', exchange: 'NASDAQ' }),
      getDividends: vi.fn(),
    } as any);
  });
  ...
  describe('createTransactionAction', () => {
    it('creates BUY transaction successfully', async () => {
      ...
      expect(txRepo.createTransaction).toHaveBeenCalledWith('acc-1', expect.objectContaining({
        type: 'BUY',
        symbol: 'AAPL',
        quantity: 10,
        unitPrice: 150
      }));
      expect(symbolRepo.upsertSymbol).toHaveBeenCalledWith('AAPL', 'Apple Inc.', 'NASDAQ');
    ...
    it('throws error if account not found', async () => {
      ...
      await expect(createTransactionAction('acc-1', {}, formData)).rejects.toThrow('Account not found');
    });
  });
  ...
  describe('deleteTransactionAction', () => {
    it('deletes transaction and redirects', async () => {
      try {
        await deleteTransactionAction('acc-1', 'tx-1');
      } catch (e) {
        expect(e).toBeInstanceOf(RedirectError);
      }
